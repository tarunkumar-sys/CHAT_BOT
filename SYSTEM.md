...
Lumi is a **local-first** AI chat application. It defaults to fully local operation using Ollama and Qdrant. However, it can seamlessly route through OpenAI or Google Gemini if their respective API keys are configured. All web scraping runs through a local Playwright Chromium instance.
...
```
Ollama  (port 11434)  — Local LLM inference + embeddings (optional fallback)
Cloud APIs            — Optional OpenAI / Gemini endpoints
Qdrant  (port 6333)   — vector storage (separated by embedding dimensions)
...
Collections are named dynamically with their corresponding vector dimensions, e.g. `{collection_name}_{dimensions}`, to prevent dimension mismatch crashes when switching models.

### `conversation_memory_{dimensions}`
...
{
  id: string,           // UUID v4
  vector: number[],     // Embedding vector (768-dim or 1536-dim depending on model)
                        // of "User: {msg}\nAssistant: {reply}"
  payload: {
    userId: string,
    userMessage: string,
...
### `user_custom_memory_{dimensions}`
...
{
  id: string,           // UUID v4
  vector: number[],     // Embedding vector of fact text
  payload: {
    userId: string,
...
```typescript
{
  ...
  vector: number[],     // Embedding vector of chunk text
  payload: {
    metadata: {
...
| `OLLAMA_MODEL` | Yes | `qwen2.5:1.5b` | Local model used for LLM inference (fallback) |
| `QDRANT_URL` | Yes | `http://localhost:6333` | Qdrant REST API URL |
| `OPENAI_API_KEY` | No | — | OpenAI API key for GPT models & embeddings |
| `OPENAI_MODEL` | No | `gpt-4o-mini` | OpenAI LLM model |
| `GEMINI_API_KEY` | No | — | Gemini API key |
| `GEMINI_MODEL` | No | `gemini-2.5-flash` | Gemini LLM model |
| `LANGCHAIN_TRACING_V2` | No | — | Set to `true` to enable LangSmith tracing |
| `LANGCHAIN_API_KEY` | No | — | LangSmith API key |
| `LANGCHAIN_PROJECT` | No | — | LangSmith project name |

Embedding models and dimensions are dynamically selected based on whether OpenAI or Gemini keys are specified, reverting to local Ollama (`nomic-embed-text`) otherwise.
